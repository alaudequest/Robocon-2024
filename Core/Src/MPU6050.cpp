...
HAL_StatusTypeDef MPU6050::Write(MPU6050_RegisterAddress reg, uint8_t data) {
	...
	uint8_t dataTemp[1];
	dataTemp[0] = data;
	return HAL_I2C_Master_Transmit(this->i2c, this->address << 1, dataTemp, 1, HAL_MAX_DELAY);
...
uint8_t MPU6050::Read(MPU6050_RegisterAddress reg) {
	...
	uint8_t data[1];
	data[0] = reg;
	HAL_I2C_Master_Transmit(this->i2c, this->address << 1, data, 1, HAL_MAX_DELAY);
	HAL_I2C_Master_Receive(this->i2c, this->address << 1, data, 1, HAL_MAX_DELAY);
	return data[0];
}

uint16_t MPU6050::GetTemperature() {
	uint16_t data = (this->Read(TEMP_OUT_H) << 8 | this->Read(TEMP_OUT_L));
	return data;
}
